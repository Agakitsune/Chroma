...
public:
  ...
  MouseCommand(const MouseCommand &cmd) noexcept
      : main(cmd.main), second(cmd.second) {}
  ...
  /**
   * @brief Discard metho,d called when the right mouse button is clicked at any
   * point during dragging. Reset the command to the beginning state and revert
   * any changes made to the layer data
   *
   * @param selection The current selection
   */
  ...
  /**
   * @brief Rendering method, called every frame to render visual to help the
   * user see what the command is doing
   *
   * @param preview A texture that is above the current selected layer, every
   * layer above the current layer will be drawn above this texture
   * @param overlay A texture that is above everything
   * @param selection The current selection
   */
  virtual void render(SDL_Texture *preview, SDL_Texture *overlay,
                      SDL_Rect &selection) noexcept = 0;
   ...
   * instead of generating a new one using @ref MouseCommand::next(), it will
   * keep on this one for future use
   *
   *
   * @return true The command is persitent
   * @return false The command isn't persitent
   *
   * @warning This method isn't use in this version, you can override it, but
   * there is no effect
   */
   ...
   *
   * Special command can transform into another command when on the current
   * selection if there is any
   *
   * eg. The Select Tool will move the current selection instead of creating
   * another one
   *
  ...
  /**
   * @brief Next command, generally it's the same type (BrushCommand ->
   * BrushCommand), but can be used to create another chained command
   *
   * @return std::unique_ptr<MouseCommand> The next command
   *
   * @warning although it is possible to return the same instance (it then won't
   * be pushed on the stack), used with @ref MouseCommand::is_persitent may
   * cause some crash
   */
