...
public:
     ...
     * @ref Signal
     *
     * Add a Signal to the application that can be used by Windows to communicate
     * with others.
     *
     * The template parameters define the type of the Signal arguments, THIS IS
     * IMPORTANT, those parameters need to match through call of `connect_signal`
     * and `emit_signal`
     *
     * ```
     * struct A {
     ...
     * @ref Signal
     *
     * Connect a member method to a signal, the argument in the function MUST
     * match the type of the Signal at creation, passing down the template
     * arguments won't work here
     *
     * * ```
     * struct A {
     *      void _on_test_color(Color c);
     ...
     * @ref Signal
     *
     * Emit a signal and call every connected method, to ensure no undefined
     * behavior, consider passing down the template arguments
     *
     * ```
     * emit_signal<Color>("test_color", BLACK); // Force down type, will emit
