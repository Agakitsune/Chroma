...
namespace chroma {
    ...
    void BrushCommand::update(const SDL_Point &point, SDL_Rect &selection) noexcept {
        SDL_FPoint p{(float)point.x, (float)point.y};
        Color c;
        uint8_t *pix = (uint8_t*)surface->pixels;
    ...
    void BrushCommand::render(SDL_Texture *preview, SDL_Texture *overlay, SDL_Rect &selection) noexcept {
