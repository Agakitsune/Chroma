...
void SelectMoveCommand::start(const SDL_Point &point,
  ...
  tex =
      SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32,
                        SDL_TEXTUREACCESS_STREAMING, selection.w, selection.h);
...
void SelectMoveCommand::update(const SDL_Point &point,
                               SDL_Rect &selection) noexcept {
  e = point;
}

void SelectMoveCommand::end(const SDL_Point &point,
                            SDL_Rect &selection) noexcept {
  update(point, selection);
}

void SelectMoveCommand::discard(SDL_Rect &selection) noexcept { e = s; }

void SelectMoveCommand::render(SDL_Texture *preview, SDL_Texture *overlay,
                               SDL_Rect &selection) noexcept {
  SDL_Renderer *renderer = App::get_renderer();

  SDL_SetRenderTarget(renderer, preview);

  SDL_FPoint delta{e.x - s.x, e.y - s.y};

  SDL_FRect rect = {selection.x + delta.x, selection.y + delta.y, selection.w,
                    selection.h};
