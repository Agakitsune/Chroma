...
/**
 * @brief Handle the popup new to ask if user confirm wanting to create new
 * canva and specify dimension, 16x16 by default
 *
 */
void NewMenuItem::display() noexcept {
  ...
  if (ImGui::BeginPopupModal("New", nullptr,
    ...
    ImGui::SameLine();
    ImGui::InputScalar("##width", ImGuiDataType_U32, &w, nullptr, nullptr,
                       "%upx"); // Need to store w and h in a better way

    ImGui::Text("Height:");

    ImGui::SetNextItemWidth(-FLT_MIN);
    ImGui::SameLine();
    ImGui::InputScalar("##height", ImGuiDataType_U32, &h, nullptr, nullptr,
                       "%upx");

    if (ImGui::Button("OK", ImVec2(140, 0))) {
      // Create new file with specified width and height
      App::get_instance()->emit_signal<uint32_t, uint32_t>(
          "create_canvas_requested", w, h);
