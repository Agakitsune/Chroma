...
void PaletteWindow::ready() noexcept {
  App::get_instance()->connect_signal("color_picked", this,
                                      &PaletteWindow::add_color);
}
/**
...
void PaletteWindow::display() noexcept {
  ...
  const uint64_t palette_height =
      palette_size / palette_width + (palette_size % palette_width ? 1 : 0);

  for (uint64_t y = 0; y < palette_height; y++) {
    const uint64_t row =
        palette_size > palette_width ? palette_width : palette_size;
    palette_size -= row;
    ...
    for (uint64_t x = 0; x < row; x++) {
      ...
      if (ImGui::InvisibleButton("##color", back_size)) {
        App::get_instance()->emit_signal<const Color &>("main_color_selected",
                                                        color);
        selected = index;
      }
      ...
      draw_list->AddRectFilled(pos, pos + back_size,
                               selected == index ? IM_COL32(255, 255, 255, 255)
                                                 : IM_COL32(0, 0, 0, 255));

      ImGui::RenderColorRectWithAlphaCheckerboard(
          draw_list, pos + ImVec2(1, 1), pos + ImVec2(1, 1) + button_size,
          color.to_u32(), 8, ImVec2(0, 0));
