...
void ColorPickerWindow::ready() noexcept
{
    App::get_instance()->connect_signal(
        "main_color_selected", this, &ColorPickerWindow::_on_main_color_selected);
    App::get_instance()->connect_signal(
...
void ColorPickerWindow::display() noexcept
{
    ...
    if (ImGui::IsItemActive() || right_clicked) {
        ...
        IM_ASSERT(g.ColorEditCurrentID != 0);
        if (!(g.ColorEditSavedID != g.ColorEditCurrentID || g.ColorEditSavedColor != ImGui::ColorConvertFloat4ToU32(ImVec4(main_color[0], main_color[1], main_color[2], main_color[3])))) {
            // return;
            H = g.ColorEditSavedHue;
        }
    ...
    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + sv_picker_size,
        ...
        col_white);
    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + sv_picker_size, 0,
        0, col_black, col_black);
    // ImGui::RenderFrameBorder(picker_pos, picker_pos + sv_picker_size,
    // 0.0f);
    sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size.x),
        picker_pos.x + 2,
        picker_pos.x + sv_picker_size.x - 2); // Sneakily prevent the circle to stick out too much
    sv_cursor_pos.y = ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size.y),
    ...
    draw_list->AddRectFilledMultiColor(
        bar1_bb.Min, bar1_bb.Max, user_col32_striped_of_alpha & ~IM_COL32_A_MASK,
