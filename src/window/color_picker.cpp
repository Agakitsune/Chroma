...
/**
 * @brief Display various elements like picker from colorpicker, color gradient,
 * hue slider, etc
 *
 */
void ColorPickerWindow::display() noexcept {
  ...
  if (ImGui::IsItemActive() || right_clicked) {
    ...
    // ImGui::ColorEditRestoreH(&main_color.r, &H); // Greatly reduces hue
    // jitter and reset to 0 when hue == 255 and color is rapidly modified using
    // SV square.
  ...
  // bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz),
  ...
  // ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width
  // + 2.0f, style.Alpha);
