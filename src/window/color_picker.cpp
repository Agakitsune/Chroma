...
namespace chroma {
    ...
    void ColorPickerWindow::ready() noexcept {
        App::get_instance()->connect_signal(
            "main_color_selected", this,
            &ColorPickerWindow::_on_main_color_selected);
        App::get_instance()->connect_signal(
            "second_color_changed", this,
            &ColorPickerWindow::_on_second_color_selected);
    }
    /**
     * @brief Display various elements like picker from colorpicker, color
     * gradient, hue slider, etc
     *
     */
    void ColorPickerWindow::display() noexcept {
        ...
        if (ImGui::IsItemActive() || right_clicked) {
            ...
            // ImGui::ColorEditRestoreH(&main_color.r, &H); // Greatly reduces
            // hue jitter and reset to 0 when hue == 255 and color is rapidly
            ...
            if (right_clicked) {
                ...
                App::get_instance()->emit_signal<const Color &>("color_picked",
                                                                main_color);
            }
        }
        ...
        if (ImGui::IsItemActive()) {
            H = ImSaturate((io.MousePos.x - picker_pos.x) /
                           (sv_picker_size.x - 1));
            value_changed_h = true;
        }

        ImGui::SetCursorScreenPos(
            ImVec2(picker_pos.x, picker_pos.y + sv_picker_size.y + bar_height));
        ImGui::InvisibleButton("alpha", ImVec2(sv_picker_size.x, bar_height));
        if (ImGui::IsItemHovered())
            CursorManager::set_cursor(Cursor::Picker);
        if (ImGui::IsItemActive()) {
            main_color[3] = ImSaturate((io.MousePos.x - picker_pos.x) /
                                       (sv_picker_size.x - 1));
            value_changed = true;
        }
        ImGui::PopItemFlag();
        ...
        ImGui::ColorConvertHSVtoRGB(H, S, V, main_color[0], main_color[1],
                                    main_color[2]);
        ...
        ImU32 user_col32_striped_of_alpha =
            ImGui::ColorConvertFloat4ToU32(ImVec4(R, G, B, 1.0));
        ...
        draw_list->AddRectFilledMultiColor(
            picker_pos, picker_pos + sv_picker_size, col_white, hue_color32,
            hue_color32, col_white);
        draw_list->AddRectFilledMultiColor(picker_pos,
                                           picker_pos + sv_picker_size, 0, 0,
                                           col_black, col_black);
        // ImGui::RenderFrameBorder(picker_pos, picker_pos + sv_picker_size,
        // 0.0f);
        sv_cursor_pos.x =
            ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size.x),
                    picker_pos.x + 2,
                    picker_pos.x + sv_picker_size.x -
                        2); // Sneakily prevent the circle to stick out too much
        sv_cursor_pos.y = ImClamp(
            IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size.y),
            picker_pos.y + 2, picker_pos.y + sv_picker_size.y - 2);
        ...
        for (int i = 0; i < 6; ++i) {
            draw_list->AddRectFilledMultiColor(
                ImVec2(picker_pos.x + i * (sv_picker_size.x / 6), bar_pos_y),
        ...
        draw_list->AddCircle(hue_bar, sv_cursor_rad + 1, col_midgrey,
                             sv_cursor_segments);
        draw_list->AddCircle(hue_bar, sv_cursor_rad, col_white,
                             sv_cursor_segments);
        // RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x
        // + bars_width, picker_pos.y + sv_picker_size), 0.0f);
        // ImGui::RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1,
        // bar0_line_y), ImVec2(bars_triangles_half_sz + 1,
        // bars_triangles_half_sz), bars_width + 2.0f, style.Alpha);

        bar_pos_y = picker_pos.y + sv_picker_size.y + bar_height;
        float alpha = ImSaturate(main_color[3]);
        ImRect bar1_bb(picker_pos.x, bar_pos_y, picker_pos.x + sv_picker_size.x,
                       bar_pos_y + bar_height);
        ImGui::RenderColorRectWithAlphaCheckerboard(
            draw_list, bar1_bb.Min, bar1_bb.Max, 0, 16.0f,
            ImVec2(0.0f, -(16.0f - bar_height / 2.0f)));
        draw_list->AddRectFilledMultiColor(
            bar1_bb.Min, bar1_bb.Max,
            user_col32_striped_of_alpha & ~IM_COL32_A_MASK,
            user_col32_striped_of_alpha, user_col32_striped_of_alpha,
            user_col32_striped_of_alpha & ~IM_COL32_A_MASK);
        // float bar1_line_y = IM_ROUND(picker_pos.y + (1.0f - alpha) *
        // sv_picker_size);

        ImVec2 alpha_bar;
        alpha_bar.y = bar_pos_y + bar_height / 2.0;
        alpha_bar.x = IM_ROUND(picker_pos.x + alpha * sv_picker_size.x);
        draw_list->AddCircle(alpha_bar, sv_cursor_rad + 1, col_midgrey,
                             sv_cursor_segments);
        draw_list->AddCircle(alpha_bar, sv_cursor_rad, col_white,
                             sv_cursor_segments);

        // RenderFrameBorder(bar1_bb.Min, bar1_bb.Max, 0.0f);
        // RenderArrowsForVerticalBar(draw_list, ImVec2(bar1_pos_x - 1,
        // bar1_line_y), ImVec2(bars_triangles_half_sz + 1,
        // bars_triangles_half_sz), bars_width
        // + 2.0f, style.Alpha);
    ...
    void ColorPickerWindow::_on_main_color_selected(const Color &clr) noexcept {
        main_color = clr;
        App::get_instance()->emit_signal<const Color &>("main_color_changed",
                                                        clr);
    }
    /**
     ...
     */
    void
    ColorPickerWindow::_on_second_color_selected(const Color &clr) noexcept {
        second_color = clr;
        App::get_instance()->emit_signal<const Color &>("second_color_changed",
                                                        clr);
    }
} // namespace chroma